...
def emitir_pdf_laudo(cod_car, mun_uf, area_ha, v_base, v_min, v_rec, status, graf_buf, data_str, geom=None, parecer="LIBERADO"):
    ...
    if parecer == "RETIDO":
        elementos.append(
            Paragraph("A verificação analítica comprova que <b>houve detecção de supressão florestal persistente após 31/12/2020</b> no perímetro do imóvel. A propriedade <b>NÃO ATENDE</b> aos critérios de conformidade do Regulamento (UE) 2023/1115 (EUDR). Recomenda-se retenção preventiva na esteira de originação.", style_body)
        )
    else:
        elementos.append(
